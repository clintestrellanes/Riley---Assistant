...
export default function AINewContainerChat({ onUpdate, project_cont, active_project }: NewContainerProps) {
  ...
  const handleApprove = (content: project_content) => {
    try {
      ...
      if (projectIndex !== -1) {
        // Generate a permanent unique ID for the new container
        const new_content: project_content = {
          ...content,
          id: Date.now() + Math.floor(Math.random() * 1000) 
        };

        // Safely prepend the new content to the existing content array
        ...
        // Update local UI state to show the checkmark
